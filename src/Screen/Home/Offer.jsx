...
function Offer() {
  ...
  const fetchHotels = async () => {
    try {
      ...
      const response = await axios.get(`https://localhost:7246/api/Offer`);
  ...
  const handleItemDelete = async (id) => {
    try {
      await axios.delete(`https://localhost:7246/api/Offer/${id}`);
            ...
            {hotels.map((hotel) => (
              <tr key={hotel.id} className="text-center">
                ...
                <td className="border px-1 py-1">
                  <img
                    src={`https://localhost:7246/resources/${hotel.imageUrl}`}
                    alt={hotel.imageUrl}
